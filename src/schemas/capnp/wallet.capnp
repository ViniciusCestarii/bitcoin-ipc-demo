...
interface Wallet $Proxy.wrap("interfaces::Wallet") {
    ...
    createTransaction @25 (context :Proxy.Context, recipients :List(Recipient), coinControl :CoinControl, sign :Bool, changePos :UInt32, hasChangePos :Bool) -> (result :Common.Result(CreatedTransactionResult));
...
struct CreatedTransactionResult $Proxy.wrap("wallet::CreatedTransactionResult")
{
    tx @0 :Data;
    fee @1 :Int64;
    feeCalc @2 :Common.FeeCalculation $Proxy.name("fee_calc");
