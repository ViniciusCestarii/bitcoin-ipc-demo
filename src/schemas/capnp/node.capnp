...
interface Node $Proxy.wrap("interfaces::Node") {
    ...
    getProxy @17 (context :Proxy.Context, net :Int32) -> (result :ProxyInfo, hasResult :Bool);
...
struct NodeStats $Proxy.wrap("CNodeStats") {
    ...
    sendBytes @12 :UInt64 $Proxy.name("nSendBytes");
    sendBytesPerMsgType @13 :List(Common.PairUInt64(Text)) $Proxy.name("mapSendBytesPerMsgType");
    recvBytes @14 :UInt64 $Proxy.name("nRecvBytes");
    recvBytesPerMsgType @15 :List(Common.PairUInt64(Text)) $Proxy.name("mapRecvBytesPerMsgType");
    permissionFlags @16 :Int32 $Proxy.name("m_permission_flags");
    pingTime @17 :Int64 $Proxy.name("m_last_ping_time");
    minPingTime @18 :Int64 $Proxy.name("m_min_ping_time");
    addrLocal @19 :Text $Proxy.name("addrLocal");
    addr @20 :Data $Proxy.name("addr");
    addrBind @21 :Data $Proxy.name("addrBind");
    network @22 :Int32 $Proxy.name("m_network");
    mappedAs @23 :UInt32 $Proxy.name("m_mapped_as");
    connType @24 :Int32 $Proxy.name("m_conn_type");
    transportType @25 :UInt8 $Proxy.name("m_transport_type;");
    sessionId @26 :Text $Proxy.name("m_session_id");
...
struct NodeStateStats $Proxy.wrap("CNodeStateStats") {
    ...
    pingWait @2 :Int64 $Proxy.name("m_ping_wait");
    heightInFlight @3 :List(Int32) $Proxy.name("vHeightInFlight");
    relayTxs @4 :Bool $Proxy.name("m_relay_txs");
    invToSend @5 :Int32 $Proxy.name("m_inv_to_send");
    lastInvSeq @6 :UInt64 $Proxy.name("m_last_inv_seq");
    feeFilterReceived @7 :Int64 $Proxy.name("m_fee_filter_received");
    addressesProcessed @8 :UInt64 $Proxy.name("m_addr_processed");
    addressesRateLimited @9 :UInt64 $Proxy.name("m_addr_rate_limited");
    addressRelayEnabled @10 :Bool $Proxy.name("m_addr_relay_enabled");
    theirServices @11 :UInt64 $Proxy.name("their_services");
    presyncHeight @12 :Int64 $Proxy.name("presync_height");
    timeOffset @13 :Int64 $Proxy.name("time_offset");
}
